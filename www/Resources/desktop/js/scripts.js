...
var tb_pathToImage = "/Resources/desktop/img/misc/TB/loadingAnimation.gif";
...
function tb_show(caption, url, imageGroup) {//function called when the user clicks on a thickbox link
  try {
    ...
    if(urlType == '.jpg' || urlType == '.jpeg' || urlType == '.png' || urlType == '.gif' || urlType == '.bmp'){//code to show images
      ...
      TB_WIDTH = (params['width']*1) || 601; //defaults to 630 if no paramaters were added to URL
      TB_HEIGHT = (params['height']*1) || 425; //defaults to 440 if no paramaters were added to URL
...
function tb_position() {
	var superman = tb_getPageSize();
	var tb_window = $('#TB_window');
	/*
	if(superman[1] < tb_window.innerHeight()){
		tb_window.css({marginTop: -15+'px'});
	}
	else{
  ...
  if(tb_window.innerHeight() + 121 > superman[1]){
	  //tb_window.css({ marginTop: '-' + parseInt((TB_HEIGHT / 2), 10) + 'px'});
	  tb_window.css({marginTop:0, top:0});
  }
}
...
/*!
* Reactive Table Hover Plugin v1.0.0
* reactive.com/
* Author Adam Phin
* Copyright 2012, reactive.com
* Released under the MIT and GPL Licenses.
*/

(function ($) {

    $.rmTableHover = function (element, options) {
        ...
        plugin.init = function () {
            ...
            var heads = (plugin.settings.allowTableHead) ? ", th" : "";

            // Bind
            $element.delegate('td' + heads, 'hover', function () {

                // Get index of selected Table Data item
                var index = $(this).index();
                
                // Deselect all Hovered Elements
                $element.find('td' + heads).removeClass(plugin.settings.hoverClass);

                // Select nothing if Table data item is set to ignore
                if( $(this).hasClass(plugin.settings.ignoreClass) ) return false;

                // Select Horizontal items aslong as they are not set to ignore
                if(plugin.settings.horizontal) $(this).parent().find('td' + heads).not("." + plugin.settings.ignoreClass).addClass(plugin.settings.hoverClass);
                
                // Select Vertical items aslong as they are not set to ignore
                if(plugin.settings.vertical) {
                    $element.find('td'+ heads).each(function(){ 
                        if( $(this).index() == index && !$(this).hasClass(plugin.settings.ignoreClass) ) { $(this).addClass(plugin.settings.hoverClass); }
                    });
                }
    ...
    $.fn.rmTableHover = function (options) {

        //iterate through the DOM elements we are attaching the plugin to
        return this.each(function () {

            //if plugin has not already been attached to the element
            if (undefined == $(this).data('rmTableHover')) {

                // create a new instance of the plugin
                var plugin = new $.rmTableHover(this, options);

                // store a reference to the plugin object
                $(this).data('rmTableHover', plugin);
    ...
    drawer: {
        init: function () {
            ...
            primesuper.drawer.$drawerBtn.click(function (e) {
                ...
                primesuper.drawer.$mapDetailWrap.stop().animate({ height: '1010px' }, { duration: 400, easing: 'easeOutCubic' });
